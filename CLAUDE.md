...
## Иерархия принципов (контекст для маршрутизации)

> Pack = вторые принципы. Downstream = третьи принципы. Подробно: `Zeroth-Principles/README.md`

При классификации кандидата — определи уровень:

| Тест | Уровень | Куда маршрутизировать |
|------|---------|----------------------|
| Работает в **любой области** (транс-дисциплинарное)? | 0-й (нулевой) | `Zeroth-Principles/README.md` |
| Работает в **любой дисциплине** (мета-онтология)? | 1-й (первый) | FPF |
| Работает в **конкретном домене**? | 2-й (второй) | Соответствующий Pack |
| Работает только в **конкретном проекте**? | 3-й (третий) | Downstream CLAUDE.md |

> Нулевые принципы (ZP.1–ZP.6) — крайне стабильны. Новые кандидаты на нулевой уровень — редкость. При обнаружении — обязательно Human-in-the-loop с повышенным вниманием.
